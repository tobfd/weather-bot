...
COLORS_BY_TEMP = {35: discord.Color.dark_red(), 30: discord.Color.red(), 25: discord.Color.dark_orange(), 20: discord.Color.orange(), 10: discord.Color.blue(), 0: discord.Color.dark_blue()}
...
async def get_view(city: str, sunrise: str, sunset: str, forecast: bool = False, original_weather: WeatherClass | None = None) -> discord.ui.View:
    if forecast:
        label = "Jetzt"
    ...
    view.add_item(ForecastButton(city, label, sunrise, sunset, style=style, weather=original_weather))
    return view


async def get_weather_embed(weather: WeatherClass, city: str, sunrise: str, sunset: str, three_hours: bool = False) -> discord.Embed:
    ...
    for temperature in COLORS_BY_TEMP.keys():
        if temp >= temperature:
            ...
            break
    embed = discord.Embed(title=f"🌤️ Wetter in {city.title()}{" in 3 Stunden" if three_hours else ""}", color=color)
    embed.add_field(name="🌡️ Temperatur", value=str(temp) + CRAD)
    embed.add_field(name="🌡️ Fühlt sich an wie", value=str(round(int(temperaturen.get("feels_like")))) + CRAD)
    embed.add_field(name="", value="")  # Kategorisierung
    embed.add_field(name="", value="", inline=False)  # Abstand von Zeile 1 bis Zeile 2
    embed.add_field(name="💧 Luftfeuchtigkeit", value=str(weather.humidity) + "%")
    embed.add_field(name="🍃 Wind Geschwindigkeit", value=str(round(int(weather.wind("km_hour").get("speed")))) + " Km/h")
    embed.add_field(name="", value="")  # Kategorisierung
    embed.add_field(name="", value="", inline=False)  # Abstand von Zeile 1 bis Zeile 2
    embed.add_field(name="🌄 Sonnenaufgang", value=sunrise)
    embed.add_field(name="🌇 Sonnenuntergang", value=sunset)
    embed.add_field(name="", value="")  # Kategorisierung
    embed.set_footer(text=translator.translate(weather.detailed_status), icon_url=weather.weather_icon_url())
    embed.set_thumbnail(url=weather.weather_icon_url())
    return embed


class Weather(ezcord.Cog, group="Weather", emoji="🌤️", description="Mit diesen Commands wirst du immer übers Wetter informiert."):
...
class ForecastButton(discord.ui.Button):
    def __init__(self, city: str, label: str, sunrise: str, sunset: str, weather: WeatherClass, style: discord.ButtonStyle | None = discord.ButtonStyle.primary):
    ...
    async def callback(self, interaction: Interaction):
        ...
        if self.style == discord.ButtonStyle.secondary:
            ...
            weather = response
            three_hours = True
        embed = await get_weather_embed(weather, self.city, sunrise=self.sunrise, sunset=self.sunset, three_hours=three_hours)
        view = await get_view(self.city, self.sunrise, self.sunset, forecast=three_hours, original_weather=self.weather)
